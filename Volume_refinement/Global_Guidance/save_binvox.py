...
outdir = './data/%s/after_refinement_binvox64'%class_name[opt.category]
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        for j in xrange(len(f)):
            ...
            with open(binvox, 'wb') as fout:
                binvox_rw.write(binvox_rw.Voxels(np.ascontiguousarray(voxel_data), dims, translate, scale, axis_order), fout)
        print('[%d/%d] mean iou img32: %f , mean iou input32: %f , mean iou input64: %f, precision: %f, recall: %f, time: %f' % 
            (i, len(dataset) / opt.batchSize, iou1.avg, iou2.avg, iou3.avg, pre.avg, rec.avg, time.time() - t0))
