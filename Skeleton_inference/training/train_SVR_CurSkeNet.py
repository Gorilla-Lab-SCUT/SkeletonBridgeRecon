...
parser.add_argument('--model_preTrained_AE', type=str, help='model path')
parser.add_argument('--model', type=str, default='', help='model path')
parser.add_argument('--num_points', type=int, default=600, help='number of points')
...
parser.add_argument('--fix_decoder', default='True', help='if set to True, on the the resnet encoder is trained')
...
dataset = ShapeNet(SVR=True, normal=False, train=True, class_choice=opt.category, npoints_line=2500,
                   npoints_square=2500)
dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize,
                                         shuffle=True, num_workers=int(opt.workers))

dataset_test = ShapeNet(SVR=True, normal=False, train=False, class_choice=opt.category, npoints_line=2500,
                        npoints_square=2500)
...
if opt.fix_decoder == 'True':
    network_preTrained_autoencoder = AE_CurSkeNet(num_points=opt.num_points, nb_primitives=opt.nb_primitives)
...
grid, lines_array, lines_adjacent_tensor = define_lines(num_points=opt.num_points, nb_primitives=opt.nb_primitives)

# Create network
network = SVR_CurSkeNet(num_points=opt.num_points, nb_primitives=opt.nb_primitives)
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        laplacian_smooth = curve_laplacian(pointsReconstructed, opt.nb_primitives, lines_adjacent_tensor)
