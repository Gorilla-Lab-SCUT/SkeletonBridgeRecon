...
class GCN(Model):
    ...
    def _loss(self):
        self.loss += mesh_loss(self.output1, self.placeholders, 3000, FLAGS.weight_edge, FLAGS.weight_normal)
        self.loss += mesh_loss(self.output2, self.placeholders, 3000, FLAGS.weight_edge, FLAGS.weight_normal)
        self.loss += mesh_loss(self.output3, self.placeholders, 3000, FLAGS.weight_edge, FLAGS.weight_normal)
        if FLAGS.vertex_chamfer:
            cd, dist1, idx1, dist2, idx2 = cd_loss(self.output3, self.placeholders)
            self.chamfer = cd
        
        # Weight decay loss
        conv_layers = range(1,15) + range(16,30) + range(31,46)
